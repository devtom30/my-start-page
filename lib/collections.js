...
Dashboards.after.update(function (userId, doc) {
...
Widgets_Collection.after.update(function (userId, doc) {
    _(_.keys(Widgets.image_fields)).each(function(key){
...
Images.allow({
    download: function(userId, fileObj) {
        ...
        return fileObj.owner==userId || Dashboards.find({$or:{ownerid:userId,public:true},background:fileObj._id}).count()>0 || Widgets_Collection.find({$or:image_fields,ownerid:userId}).count()>0;
    },
...
Ground.Collection(Images.files, {
    cleanupLocalData: false
});
